...
// 2️⃣ Dynamically import React wrapper (client-only)
const Line = dynamic(
    ...
    const fetchData = async () => {
      try {
        ...
        tanks.forEach(name => {
          ...
          // Sort entries by DateFerm ascending
          const sorted = entries
            .map(e => ({ ...e, parsed: parseDate(e['DateFerm']) }))
            .filter(e => e.parsed > 0)
          ...
          map[name] = {
            ...
            isEmpty,
            baseAvgOE,
            historyDates: history.map(h => h.date.toLocaleDateString('en-AU')),
            historyGravities: history.map(h => h.g),
            pHValue,
            bbtVol,
            carb,
            dox,
      ...
      <div style={{
        ...
        {tankData.map(t => {
          const {
            ...
            baseAvgOE, historyDates, historyGravities,
          ...
          const dexIncrement = HL > 0 ? (1.3 / HL) * dexCount : 0
          const currentOE = baseAvgOE !== null ? baseAvgOE + dexIncrement : null
          const currentAE = historyGravities.length
            ? historyGravities[historyGravities.length-1]
            : null
          const leg = (currentOE !== null && currentAE !== null)
            ? calcLegacy(currentOE, currentAE)
            : null
          const neu = (currentOE !== null && currentAE !== null)
            ? calcNew(currentOE, currentAE)
            : null
          const avgABV = (leg!==null && neu!==null)
            ? ((leg+neu)/2).toFixed(1)
            : null

          // Chart data
          const chartLabels = currentOE!==null
            ? ['OG', ...historyDates]
            : [...historyDates]
          const chartData = currentOE!==null
            ? [currentOE, ...historyGravities]
            : [...historyGravities]
                ...
                <>
                  ...
                  {chartData.length > 1 && (
                    <Line
                      ...
                      height={150}
                      onClick={()=>setModalChart({labels:chartLabels, data:chartData})}
                    />
                  )}
                </>
              )}
